...
The one deliberately expressive layer is the animated background: a faint "Signal Network" of connected nodes fixed behind every page — most nodes breathe quietly, a few blink brand-red like an occasional live ping, and a few edges carry a traveling dash, reading as a network of channels being watched rather than decoration for its own sake. Layered with soft drifting red glow blobs and a slow rotating conic wash behind Dashboard cards. This is confined to decoration — it never sits behind readable text or interactive controls, and it always respects `prefers-reduced-motion`.
